#!/usr/bin/env bash
# ============================================================================
#  Maqueen App Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/app-09-maqueen-app}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.maqueenapp}"
AVD_NAME="${AVD_NAME:-maqueen_app_test}"
...
cmd_test() {
  ...
  local out="/tmp/maqueen-app-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Maqueen App Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
